...
export default function FeedbackListPage() {
  ...
  const deleteFeedback = async (id) => {
    if (window.confirm("Bạn có chắc muốn xóa góp ý này? Sự kiện gọi Soft Delete ở Backend.")) {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/suggestions/${id}`, {
          method: "DELETE",
        });
        if (res.ok) {
  ...
  const submitReply = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/suggestions/${replyModalData.id}/reply`, {
        method: "PUT",
